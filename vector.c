...
void tv_Vector2Normalize(tv_Vector2 v, tv_Vector2* result)
{
	...
	/* assert(mag != 0.0f); */
	if(mag == 0.0f) {
		puts("warning: attempted to normalize a vector with magnitude 0");
		return;
	}
	result->x = v.x / mag;
	result->y = v.y / mag;
...
{
	return (float)sqrt((v2.x*v2.x)-(v1.x*v1.x) + (v2.y*v2.y)-(v1.y*v1.y));
}
float tv_Vector3Distance(tv_Vector3 v1, tv_Vector3 v2)
{
	return (float)sqrt((v2.x*v2.x)-(v1.x*v1.x) + (v2.y*v2.y)-(v1.y*v1.y) + (v2.z*v2.z)-(v1.z*v1.z));
}
float tv_Vector4Distance(tv_Vector4 v1, tv_Vector4 v2)
{
	return (float)sqrt((v2.x*v2.x)-(v1.x*v1.x) + (v2.y*v2.y)-(v1.y*v1.y) + (v2.z*v2.z)-(v1.z*v1.z) + (v2.w*v2.w)-(v1.w*v1.w));
}
/******************************************************************************
 * LERP
******************************************************************************/
